...
static void motor_run_three_seconds(bool forward)
{
    ...
    // 0 -> 20 -> 40 -> ... -> 3980
    //================================================
    for(duty = 0;duty < MOTOR_DUTY_MAX;duty += MOTOR_DUTY_STEP)
    {
    ...
    // 4000 -> 3980 -> ... -> 20
    //================================================
    for(duty = MOTOR_DUTY_MAX; duty > 0;duty -= MOTOR_DUTY_STEP)
    {
        pwm_set_duty(MOTOR_PWM, duty);
...
int core0_main(void)
{
    ...
    gpio_init(MOTOR_DIR, GPO,GPIO_LOW,GPO_PUSH_PULL);
    //================================================
    // PWM 初始化
    //
    // P21_5
    ...
    pwm_init(MOTOR_PWM,MOTOR_PWM_FREQ,0);
